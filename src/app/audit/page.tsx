...
import {
  detectFollowersAndFollowing,
  type DetectionResult
} from "../../lib/instagram/detect";
import { parseStringListData } from "../../lib/instagram/parsers";
import {
  computeRelationshipSets,
  type ComputeResult
} from "../../lib/instagram/compute";
import {
  getCredibility,
  setCredibility,
  getCredibilityScore,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
      <div className="w-full max-w-lg rounded-2xl border border-red-500/30 bg-[#0a0a0a] p-6 shadow-2xl">
        <h3 className="mb-2 text-xl font-bold text-white">Generate Unfollow Plan</h3>

        <div className="mb-6 rounded-lg border border-red-500/20 bg-red-500/10 p-4 text-sm text-red-200">
          <strong>CRITICAL WARNING:</strong> We do NOT log in to Instagram. We do NOT
          automate unfollowing. You must perform these actions manually inside the
          Instagram app. Using automation tools can get your account banned.
        </div>

        <p className="mb-4 text-sm text-white/70">
          You have selected <strong>{selectedUsers.length}</strong> accounts. Choose how
          you want to export this list:
        </p>
...
export default function AuditPage() {
  ...
  const [sortOrder, setSortOrder] = useState<"score_asc" | "score_desc" | "alpha">(
    "score_asc"
  );
  ...
  const visibleNotFollowingBack = useMemo(() => {
    ...
    // Sort
    // We include credibilityTick to ensure refresh when tags change
    return [...list].sort((a, b) => {
      void credibilityTick;
  ...
  const toggleSelectAll = () => {
    ...
    const allVisibleSelected = visibleNotFollowingBack.every((p) =>
      ...
      <div className="mx-auto max-w-6xl space-y-8">
        <header>
          ...
          <p className="text-white/60">
            Upload, analyze, and tag your Instagram connections.
          </p>
        </header>

        {/* Upload Section */}
        {!zipResult && <UploadZip onUploadComplete={handleUploadComplete} />}
        ...
        {zipResult && detection && (
          <div className="rounded-xl border border-white/10 bg-white/5 p-6 flex flex-wrap gap-x-8 gap-y-4 items-end">
            <div className="w-full text-sm text-white/70 mb-2">
              <p>
                We tried to auto-detect your files. If the score is low ({"<"}50) or the
                results look wrong (0 mutuals), please manually select the correct JSON
                files from the list.
              </p>
            </div>
        ...
        {computeResult && (
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <Card
                title="Followers"
                count={computeResult.stats.followersCount}
                subtitle="Total detected"
              />
              <Card
                title="Following"
                count={computeResult.stats.followingCount}
                subtitle="Total detected"
              />
              <Card
                title="Mutuals"
                count={computeResult.stats.mutualsCount}
                subtitle={`${computeResult.stats.mutualsPercentage.toFixed(1)}% of following`}
              />
              <Card
                title="Not Following Back"
                count={computeResult.stats.notFollowingBackCount}
                subtitle={`${computeResult.stats.notFollowingBackPercentage.toFixed(1)}% of following`}
              />
            </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                <h2 className="text-xl font-semibold">
                  ...
                  <span className="text-white/40 text-base">
                    ({visibleNotFollowingBack.length})
                  </span>
                </h2>
                <div className="flex gap-2">
                  <button
                    onClick={downloadCsv}
                    className="px-4 py-2 bg-white text-black text-sm font-bold rounded-lg hover:bg-white/90"
                  >
                    Download CSV (All)
                  </button>
                </div>
                      ...
                      <th className="w-10 px-4 py-3">
                        <input
                          ...
                          checked={
                            ...
                            visibleNotFollowingBack.every((p) =>
                              selectedUsernames.has(p.username)
                            )
                          }
                  ...
                  <tbody className="divide-y divide-white/5">
                    {paginatedList.map((p) => {
                      ...
                      return (
                        <tr
                          key={p.username}
                          className={`hover:bg-white/5 ${isSelected ? "bg-white/[0.03]" : ""}`}
                        >
          ...
          <div className="pt-8">
            <button
              onClick={() => {
                if (
                  confirm(
                    "Are you sure? This will clear all local tags, scores, and reset the current session."
                  )
                ) {
